...
fun RiveDemoScreen(
    ...
    LaunchedEffect(Unit) {
        while (true) {
            delay(60000L) // 1分ごとにGCを促進
            Log.d("RiveMemory", "メモリクリーンアップを促進します")
            gc() // GCを促進
        }
    }
    ...
    //riveViewRef.value?.enablePointerEvents()
    ...
    LaunchedEffect(Unit) {
        ...
        while (true) {
            ...
            riveViewRef.value?.let { riveView ->
                // Yは画面中央の適当な位置
                riveView.lookAt(xPos, Resources.getSystem().displayMetrics.heightPixels / 2f, stateMachineName)
            }
        }
    }
